...
class LogisticRegression():
    ...
    def gradient_descent(self, x, y):
        ...
        Args:
            x: has to be an numpy.ndarray,
                a matrix of shape eleves(m) * matiere(n).
            y: has to be an numpy.ndarray,
                a vector of shape m * 1.
        Returns:
            new_theta: numpy.array,
                a vector of dimension (number of features + 1, 1).
        Raises:
    ...
    def stochastic_gradient_descent(self, x, y):
        ...
        m, n = x.shape
        for _ in range(self.max_iter):
            # split des data en mini batch a chaque iteration les batch son
            # different
